...
try:
    ...
    # --- OTHER TABS ---
    with tabs[2]:
        st.download_button("📥 Excel Download", convert_df_to_excel(export_df), "Rosters.xlsx")
    ...
    with tabs[4]:
        scout_n = st.text_input("Scout Player:")
        if scout_n: st.markdown(get_gm_advice("Scouting Fit Analysis", scout_n))
    with tabs[5]:
